...
Скрипт парсит отчёты двух платёжных систем, сверяет транзакции и экспортирует результат в Excel.
...
- **Подключить БД** — раскомментировать `save_*()` в `main.py`, модуль `db.py` уже готов
- **Сменить формат экспорта** — только `export.py`, остальное не трогать
...
- **Ведущие нули в RRN** сохраняются через `zfill(6)` — `'63534'` → `'026119'` без этого сломает сверку
- **Время транзакции в RpaBank отсутствует** — сверка идёт только по дате (`YYYYMMDD`), поэтому сравнивается с `Transaction Date` из Pindodo, а не с `Local Transaction Date and Time`
- **Terminal ID может содержать буквы** — например `ATM19104`, парсер это учитывает
- **Формат суммы** — разделитель может быть как `.` так и `,`, нормализуется при парсинге
- **Дубликаты** — при наличии одинаковых ключей сверки в словаре побеждает последняя запись
